...
public class KitsuneChatUtils {
	...
	public String formatChatPrefixes(String target, String formatString, AsyncPlayerChatEvent context) {
		...
		if (plugin.getConfig().isString("worldalias."+context.getPlayer().getWorld().getName())) {
			output = output.replaceAll("\\{world\\}", plugin.getConfig().getString("worldalias."+context.getPlayer().getWorld().getName()));
		} else {
